tell @a[tag=automation_debug] run/slot7

function automation:blockplacer/run/place

scoreboard players reset @s automation

execute store result score @s automation run data get block ~ ~ ~ Items[7].Count
scoreboard players operation @s automation -= #1 automation
execute store result block ~ ~ ~ Items[7].Count byte 1 run scoreboard players get @s automation
